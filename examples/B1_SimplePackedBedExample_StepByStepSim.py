...
def reactionNetworkCallbackWithPackedBed(Cs, index, theta, packedBedQuality):
    ...
    if index == 0: return -1*rate # Consumption of C1
    if index == 1: return rate # Formation of C2

    if index == -1: return -.1*rate  # Packed bed quality decrease
...
def main():
    ...
    startTimes = np.linspace(0, timeVec[-1], 5)

    for i in range(len(startTimes)-1):
        print(f"--- Step {i+1} ---")
        reactor.simulateStep(startTime=startTimes[i], endTime=startTimes[i+1], Cin=Cin, timeVec=timeVec, flowRate=flowRate, temperature=temperature, isVolumetricFlowRate=False)
        reactor.plot(Cin=Cin, CinTimeVec=timeVec, showPlot=i == len(startTimes)-2)
